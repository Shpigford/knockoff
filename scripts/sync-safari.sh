...
# Run after changing manifest.json, src/, data/, options/, onboarding/, or
# icons/, then rebuild safari/Knockoff/Knockoff.xcodeproj.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
RES_DIR="$ROOT_DIR/safari/Knockoff/Knockoff Extension/Resources"

rsync -a --delete "$ROOT_DIR/src" "$ROOT_DIR/data" "$ROOT_DIR/options" "$ROOT_DIR/onboarding" "$ROOT_DIR/icons" "$RES_DIR/"
